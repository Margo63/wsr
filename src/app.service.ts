...
export class AppService {
    ...
    //запрос на изменение пароля(получение кода валидации), сервер получает email
    getValidCode(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);
    ...
    sendValidCodeAgain(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);
    ...
    checkValidCode(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);
    ...
    changePassword(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);

        if (index !== -1) {
            DATA[index].password = params.password
            fs.writeFile('src/data.json', JSON.stringify(DATA), (err) => {
                if (err) throw err;
            });
            return {"message": "success"}
    ...
    getBalance(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);
    ...
    getUUID(body: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);

        //если есть все неоюхожимые параметры, то вохвращается UUID
        if (index !== -1 && body.origin_details && body.delivery_details && body.package_details &&
            body.origin_details.address != "" && body.origin_details.state != "" && body.origin_details.country != "" && body.origin_details.phone != "" &&
            body.delivery_details.address != "" && body.delivery_details.state != "" && body.delivery_details.country != "" && body.delivery_details.phone != "" &&
            body.package_details.weight != "" && body.package_details.items != "" && body.package_details.price != ""
        ) {
            let uuid = "R-" + uuidv4()
            let obj = {
                "id": uuid,
    ...
    //история операций
    getHistoryOfOperations(params: any, token: string): any {
        const index = DATA.map((g) => {
            return g.token;
        }).indexOf(token);
    ...
    getDeliverymen(token: string):any{
        ...
        if(index!==-1){
            if(DATA[index].packages)
                return DATA[index].packages.map( e => DELIVERYMAN[e.deliveryman] )
        }
    }
